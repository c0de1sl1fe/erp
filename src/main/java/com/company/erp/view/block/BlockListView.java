package com.company.erp.view.block;
...
@ViewController(id = "Block.list")
@ViewDescriptor(path = "block-list-view.xml")
...
public class BlockListView extends StandardListView<Block> {
    ...
    @ViewComponent
    private CollectionContainer<Block> blocksDc;

    @ViewComponent
    private InstanceContainer<Block> blockDc;

    @ViewComponent
    private InstanceLoader<Block> blockDl;

    @ViewComponent
    private VerticalLayout listLayout;

    @ViewComponent
    private DataGrid<Block> blocksDataGrid;
    ...
    public void onInit(final InitEvent event) {
        blocksDataGrid.getActions().forEach(action -> {
    ...
    @Subscribe("blocksDataGrid.createAction")
    public void onBlocksDataGridCreateAction(final ActionPerformedEvent event) {
        prepareFormForValidation();

        dataContext.clear();
        Block entity = dataContext.create(Block.class);
        blockDc.setItem(entity);
        updateControls(true);
    }

    @Subscribe("blocksDataGrid.editAction")
    public void onBlocksDataGridEditAction(final ActionPerformedEvent event) {
        updateControls(true);
    }
    ...
    @Subscribe(id = "blocksDc", target = Target.DATA_CONTAINER)
    public void onBlocksDcItemChange(final InstanceContainer.ItemChangeEvent<Block> event) {
        prepareFormForValidation();

        Block entity = event.getItem();
        dataContext.clear();
        if (entity != null) {
            blockDl.setEntityId(EntityValues.getId(entity));
            blockDl.load();
        } else {
            blockDl.setEntityId(null);
            blockDc.setItem(null);
        }
        updateControls(false);
    }
    ...
    private ValidationErrors validateView(Block entity) {
    ...
    private void updateControls(boolean editing) {
        ...
        blocksDataGrid.getActions().forEach(Action::refreshState);
