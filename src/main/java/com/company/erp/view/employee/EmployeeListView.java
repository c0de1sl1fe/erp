package com.company.erp.view.employee;
...
public class EmployeeListView extends StandardListView<Employee> {
    ...
    public void onInit(final InitEvent event) {
        employeesDataGrid.getActions().forEach(action -> {
    ...
    @Subscribe("employeesDataGrid.createAction")
    public void onEmployeesDataGridCreateAction(final ActionPerformedEvent event) {
        prepareFormForValidation();

        dataContext.clear();
        Employee entity = dataContext.create(Employee.class);
        employeeDc.setItem(entity);
        updateControls(true);
    }

    @Subscribe("employeesDataGrid.editAction")
    public void onEmployeesDataGridEditAction(final ActionPerformedEvent event) {
        updateControls(true);
    }
    ...
    public void onEmployeesDcItemChange(final InstanceContainer.ItemChangeEvent<Employee> event) {
        ...
        Employee entity = event.getItem();
        dataContext.clear();
        if (entity != null) {
            employeeDl.setEntityId(EntityValues.getId(entity));
            employeeDl.load();
        } else {
            employeeDl.setEntityId(null);
            employeeDc.setItem(null);
        }
        updateControls(false);
    }
    ...
    private OperationResult saveEditedEntity() {
        Employee item = employeeDc.getItem();
    ...
    private ValidationErrors validateView(Employee entity) {
    ...
    private void updateControls(boolean editing) {
        ...
        employeesDataGrid.getActions().forEach(Action::refreshState);
